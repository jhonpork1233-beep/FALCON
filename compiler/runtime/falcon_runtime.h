...
/**
 * Concatenate two C strings and return a runtime-owned temporary result.
 * Result pointer is valid until the next concat call.
 */
 ...
 */
bool falcon_str_is_empty(const char* s);
...
/**
 * Find first occurrence of needle in haystack at or after start index.
 * Returns byte index or -1 when not found.
 */
int64_t falcon_str_find_from(const char* haystack, const char* needle, int64_t start);

/**
 * Return substring [start, start+length) clamped to string bounds.
 * Result pointer is runtime-owned temporary.
 */
const char* falcon_str_substr(const char* s, int64_t start, int64_t length);

/**
 * Replace every target occurrence with replacement.
 * Result pointer is runtime-owned temporary.
 */
const char* falcon_str_replace_all(const char* input, const char* target, const char* replacement);
...
/**
 * Extract up to max_items string values from JSON string properties matching key.
 * Result is newline-delimited bullet list.
 */
const char* falcon_str_json_extract_values(const char* json, const char* key, int64_t max_items);
 ...
 * Prints the output of `ollama list` directly to stdout.
 */
 ...
 * Result pointer is runtime-owned and valid until the next capture call.
 */
const char* falcon_os_exec_capture(const char* command);

/**
 * Execute a shell command and stream stdout to current stdout.
 * Returns process exit status, or -1 on spawn failure.
 */
 ...
 */
long long falcon_file_read(const char* filename, char* buffer, long long max_size);
 ...
 */
long long falcon_file_write(const char* filename, const char* data, long long size);

/**
 * Append data to file.
 * Returns bytes written, or -1 on error.
 */
long long falcon_file_append(const char* filename, const char* data, long long size);
